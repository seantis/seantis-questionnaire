...
try:
    ...
    from utils.pyparsing import alphanums
    from utils.pyparsing import alphas
    from utils.pyparsing import Dict
    from utils.pyparsing import Group
    from utils.pyparsing import opAssoc
    from utils.pyparsing import operatorPrecedence
    from utils.pyparsing import Optional
    from utils.pyparsing import ParseException
    from utils.pyparsing import QuotedString
    from utils.pyparsing import Suppress
    from utils.pyparsing import Word
    from utils.pyparsing import ZeroOrMore
...
def __make_parser():
    ...
    return Dict(
        ZeroOrMore(
            Group(key + Optional(Suppress("=") + value, default=True))
        )
    )
...
def parse_checks(string):
    ...
    >>> parse_checks(
        'dependent=5a,no dependent="5a && 4a" dog="Roaming Rover" '
        'name=Robert foo bar'
    )
    (
        [
...
class Checker(object):
    """
    Simple wrapper to call a specific function, passing in args and kwargs
    each time
    """
